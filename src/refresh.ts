...
import { discoverCheckout, listWorkspaces, publishTokens } from './herdr';
...
export async function refresh(config: Config, preview: boolean, run: Runner = runCommand): Promise<RefreshResult[]> {
  const results: RefreshResult[] = [];
  for (const workspace of await listWorkspaces(run)) {
    ...
    try {
      ...
      // result after a switch; the next cycle will query the current branch.
      const currentBranch = (await run(['git', 'branch', '--show-current'], checkout.cwd)).trim();
      if (currentBranch !== branch) {
        results.push({ ...base, status: 'skipped', reason: 'Branch changed during lookup; retry on next refresh' });
        continue;
      }
      ...
      results.push({ ...base, status: 'error', reason: error instanceof Error ? error.message : String(error) });
    }
  }
  return results;
}
